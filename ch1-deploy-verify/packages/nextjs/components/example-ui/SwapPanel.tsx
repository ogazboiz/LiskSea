...
export const SwapPanel = () => {
  ...
  // Approve functions - dynamic based on input amount
  const { writeAsync: approveTokenA } = useScaffoldContractWrite({
    contractName: "MyToken",
    functionName: "approve",
    args: [dexAddress as `0x${string}`, inputAmount ? parseUnits(inputAmount, 18) : 0n],
  });

  const { writeAsync: approveTokenB } = useScaffoldContractWrite({
    contractName: "SimpleUSDC",
    functionName: "approve",
    args: [dexAddress as `0x${string}`, inputAmount ? parseUnits(inputAmount, 6) : 0n],
  });
  ...
  const handleApprove = async () => {
    if (!inputAmount || parseFloat(inputAmount) <= 0) {
      notification.error("Please enter a valid amount first");
      return;
    }
    ...
    try {
      if (isTokenAInput) {
        await approveTokenA();
        notification.success(`${inputAmount} ${symbolA} approved!`);
        setTimeout(() => refetchAllowanceA(), 2000);
      } else {
        await approveTokenB();
        notification.success(`${inputAmount} ${symbolB} approved!`);
