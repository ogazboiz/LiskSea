...
export const LiquidityPanel = () => {
  ...
  // Get DEX contract address directly
  ...
  useEffect(() => {
    if (amountA && amountB && allowanceA && allowanceB) {
      ...
      console.log("MTK allowance:", allowanceA.toString(), ">= amount:", amountABN.toString(), "?", allowanceA >= amountABN);
      console.log("sUSDC allowance:", allowanceB.toString(), ">= amount:", amountBBN.toString(), "?", allowanceB >= amountBBN);
  ...
  // Approve functions - dynamic based on input amounts
  const { writeAsync: approveTokenA } = useScaffoldContractWrite({
    contractName: "MyToken",
    functionName: "approve",
    args: [dexAddress as `0x${string}`, amountA ? parseUnits(amountA, 18) : 0n],
  });

  const { writeAsync: approveTokenB } = useScaffoldContractWrite({
    contractName: "SimpleUSDC",
    functionName: "approve",
    args: [dexAddress as `0x${string}`, amountB ? parseUnits(amountB, 6) : 0n],
  });
  ...
  const handleApproveA = async () => {
    if (!amountA || parseFloat(amountA) <= 0) {
      notification.error("Please enter a valid amount for Token A first");
      return;
    }
    try {
      await approveTokenA();
      notification.success(`${amountA} ${symbolA} approved!`);
  ...
  const handleApproveB = async () => {
    if (!amountB || parseFloat(amountB) <= 0) {
      notification.error("Please enter a valid amount for Token B first");
      return;
    }
    try {
      await approveTokenB();
      notification.success(`${amountB} ${symbolB} approved!`);
  ...
  const handleAddLiquidity = async () => {
    ...
    if (balanceA && balanceA < amountABN) {
      notification.error(`Insufficient ${symbolA} balance. You have ${formatBalance(balanceA, 18)} ${symbolA}`);
      return;
    }
    
    if (balanceB && balanceB < amountBBN) {
      notification.error(`Insufficient ${symbolB} balance. You have ${formatBalance(balanceB, 6)} ${symbolB}`);
      return;
    }
    ...
    try {
      ...
      console.log("MTK amount:", amountA, "->", amountABN.toString());
      console.log("sUSDC amount:", amountB, "->", amountBBN.toString());
      console.log("MTK balance:", balanceA?.toString());
      console.log("sUSDC balance:", balanceB?.toString());
      console.log("Allowance A:", allowanceA?.toString());
      console.log("Allowance B:", allowanceB?.toString());

      // Double-check allowances are sufficient
      if (allowanceA && allowanceA < amountABN) {
        notification.error(`Insufficient ${symbolA} allowance. Please approve more ${symbolA}.`);
        return;
      }
      
      if (allowanceB && allowanceB < amountBBN) {
        notification.error(`Insufficient ${symbolB} allowance. Please approve more ${symbolB}.`);
        return;
      }

      // Refresh allowances before attempting
      console.log("Refreshing allowances...");
              ...
              >
                🔄 Refresh
                  ...
                  <button 
                    ...
                    onClick={() => {
                      console.log("=== DEBUGGING ALLOWANCES ===");
                      console.log("DEX Address:", dexAddress);
                      console.log("User Address:", connectedAddress);
                      console.log("MTK Allowance:", allowanceA?.toString());
                      console.log("sUSDC Allowance:", allowanceB?.toString());
                      console.log("MTK Balance:", balanceA?.toString());
                      console.log("sUSDC Balance:", balanceB?.toString());
                    }}
                  >
